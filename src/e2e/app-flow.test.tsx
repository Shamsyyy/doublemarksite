...
describe("app end-to-end flow", () => {
  it("supports register -> checkout -> account -> download", async () => {
    ...
    await user.type(screen.getByLabelText("Подтверждение пароля"), "secure-pass-123");
    ...
    await user.click(screen.getByRole("checkbox", { name: /согласие на обработку персональных данных/i }));
    await user.click(screen.getByRole("checkbox", { name: /публичную оферту/i }));
    await user.click(screen.getByRole("button", { name: "Создать аккаунт" }));

    expect(await screen.findByText(/проверьте почту/i)).toBeInTheDocument();

    view.unmount();
    window.history.pushState({}, "", "/confirm-email?token=confirm:flow@example.com");
    render(<App />);
    expect(
      await screen.findByRole("heading", { name: "Вход" }, { timeout: 3000 }),
    ).toBeInTheDocument();

    await user.type(screen.getByLabelText("Email"), "flow@example.com");
    await user.type(screen.getByLabelText("Пароль"), "secure-pass-123");
    ...
    expect(
      await screen.findByRole("heading", { name: "Результат оплаты" }, { timeout: 5000 }),
    ).toBeInTheDocument();
    expect(await screen.findByText(/лицензия организации активирована/i)).toBeInTheDocument();

    await user.click(screen.getByRole("link", { name: "В личный кабинет" }));
    expect(await screen.findByRole("heading", { name: "Личный кабинет" })).toBeInTheDocument();
